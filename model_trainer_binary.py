...
def text_to_wordlist(text, remove_stopwords=False, Lemmatizer=False):
    ...
    for token in text.split(" "):
        ...
        #    new_token = ["What", "the", "fuck"]
        #    words += new_token
        #    print "token wtf: ", token
        #elif token.lower() in ["stfu"]:
        #    new_token = ["shut", "the", "fuck", "up"]
        #    words += new_token
        #    print "token stfu: ", token
        if glove.get(token) is None:  ####################################################### Things to do : add spell corrector
...
def build_checkpoint(model_path='best_model.h5', save_best_only=True, save_weights_only=True):
    model_checkpoint = ModelCheckpoint(model_path, save_best_only=save_best_only, save_weights_only=save_weights_only, monitor="val_auc_score", mode="max")
...
def build_model(nb_words, embedding_matrix, embedding_dimension=300, max_sequence_length=150, num_lstm=300,
    ...
    #lstm_layer_1 = LSTM(500, stateful=False, dropout=rate_drop_lstm, recurrent_dropout=recurrent_dropout, return_sequences=return_sequences)
    ...
    preds = Dense(number_class, activation='sigmoid')(conc)
...
def save_training_prediction_corrected(x_train, model, train_df, class_name, training_file_path, batch_size=1024, output_path="train_prediction.csv",):
    print "SAVE CODE CHANGE SUCCESSFULLY !!!"
    ## save train prediction to csv
    y_train_pred = model.predict([x_train], batch_size=batch_size, verbose=1)
    new_name = class_name + "_pred"
    train_df_pred = train_df
    train_df_pred[new_name] = y_train_pred
    train_df_pred.to_csv(output_path, index=True)
    print "Training file predicted {}".format(output_path)
...
def main():
    ...
    parser.add_argument('--continue', dest="RESUME_LEARNING", action='store_true', help="Continue training on top on existing model")
    parser.add_argument('--score-training', dest="SCORE", action='store_true', help="Score the training dataset using the model from base_model path")
    parser.add_argument('--base-model', dest="BASE_MODEL", default="~/", type=str, help="Base model path to initialize the initial weight")
    ...
    parser.add_argument('--model', dest="MODEL_NAME", default="lstm-max-poll", type=str, help="Specify which model topology to choose use for model training")
    ...
    x_train, test_data, word_index = tokenize_input_data(train_df, test_df, LIST_CLASSES, comment_column=comment_column, max_nb_words=max_nb_words, max_sequence_length=max_sequence_length)
    ...
    model = build_model(nb_words, embedding_matrix, embedding_dimension=embedding_dimension, max_sequence_length=max_sequence_length, num_lstm=num_lstm,
                        rate_drop_lstm=rate_drop_lstm, recurrent_dropout=recurrent_dropout, rate_drop_dense=rate_drop_dense, num_dense=num_dense,
                        return_sequences=True, trainable=False, number_class=1)

    if args.RESUME_LEARNING:
        base_model_weight = args.BASE_MODEL
        model.load_weights(base_model_weight)

    validation_split = args.VALIDATION_SPLIT
    df_submission = create_submission(sample_path=args.SAMPLE_PATH)
    ...
    logging.info("Training classes: {}, type {}".format(class_list, type(class_list)))
    for class_name in class_list:
        # sample train/validation data
        y_train = get_train_labels(train_df, [class_name])
        data_train, labels_train, data_val, labels_val = get_val_data(x_train, y_train, validation_split=validation_split)

        if args.SCORE:
            base_model_weight = args.BASE_MODEL
            model.load_weights(base_model_weight)
            out_train_predict = os.path.join(working_dir, "pred_train.csv")
            save_training_prediction_corrected(x_train, model, train_df, class_name, training_file_path, batch_size=1024, output_path=out_train_predict)
            return
        ...
        out_model_path = os.path.join(working_dir, "{}_best_model.h5".format(class_name))
        model_checkpoint = build_checkpoint(model_path=out_model_path, save_best_only=True, save_weights_only=True)

        csv_file = os.path.join(working_dir, "{}_metrics.log".format(class_name))
        ...
        if args.REDUCE_LEARNING:
            reduce_learning = ReduceLROnPlateau(monitor=monitor, factor=factor, patience=patience, min_lr=0.00001)
            callbacks.append(reduce_learning)

        logging.info("Fitting Model: {}".format(class_name))
        # Train the model
        ...
        logging.info("Best Validation score {}: {}".format(class_name, bst_val_score))

        y_test = model_predict(model, out_model_path, test_data, batch_size=1024, verbose=1)
        write_submission(df_submission, y_test, class_name)

    STAMP = str('%.4f_' % (bst_val_score))
    out_test_predict = os.path.join(working_dir, "{}_pred_test.csv".format(STAMP))
    dump_submission(df_submission, output_path=out_test_predict)
        # Test the Model
        # STAMP = str('%.4f_' % (bst_val_score))
        # out_test_predict = os.path.join(working_dir, "{}_pred_test_{}.csv".format(STAMP, class_name))
